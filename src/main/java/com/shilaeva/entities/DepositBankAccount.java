...
public class DepositBankAccount implements BankAccount, ClockSubscriber {
    ...
    /**
     * Constructor of the class DepositBankAccount.
     * @param id the id of the DepositBankAccount.
     * @param client the client of the DepositBankAccount.
     * @param clock the clock with current time.
     * @param endDate the end date of the DepositBankAccount.
     * @param percent the percent of the DepositBankAccount.
     * @param money the money of the DepositBankAccount.
     * @param limitAbove the limit above of the DepositBankAccount.
     */
    public DepositBankAccount(@NonNull UUID id, @NonNull Client client, @NonNull Clock clock,
                              @NonNull LocalDateTime endDate, @NonNull Percent percent, @NonNull BigDecimal money,
                              @NonNull LimitAbove limitAbove) {
        this.id = id;
        this.client = client;
    ...
    /**
     * Method to get transactionsHistory of the DepositBankAccount.
     * @return transactionsHistory of the DepositBankAccount.
     */
    ...
    /**
     * Method to put money in this DepositBankAccount.
     * @param transaction the transaction with money to put in this DepositBankAccount.
     */
    ...
    /**
     * Method to withdraw money from this DepositBankAccount.
     * @param transaction the transaction with money to withdraw from this DepositBankAccount.
     */
    ...
    /**
     * Method to cancel transaction and put money in this DepositBankAccount.
    ...
    /**
     * Method to cancel transaction and withdraw money from this DepositBankAccount.
     * @param transaction the transaction which should be to cancel.
     */
    ...
    /**
     * Method to check if it is possible to withdraw money from this DepositBankAccount.
     * @param transaction the transaction with money to withdraw from this DepositBankAccount.
     * @throws BankAccountException if the DepositBankAccount is not active or if new sum of money is going beyond the
     * limit above.
     */
    public void checkPutMoneyInTheAccount(@NonNull Transaction transaction) {
        if (isPeriodEnded(transaction.getDateTime())) {
     ...
     * Method to check if it is possible to withdraw money from this DepositBankAccount.
     * @param transaction the transaction with money to withdraw from this DepositBankAccount.
     * @throws BankAccountException if the period of the DepositBankAccount is not ended or if new sum of money is going
    ...
    /**
     * Method to update the DepositBankAccount and add percent income to the account.
    ...
    public void update(@NonNull LocalDateTime newDateTime) {
        ...
        if (!isPeriodEnded(timeToAddPercent)) {
            ...
            lastDateUpdate = newDateTime;
        } else {
            if (lastDateUpdate.isBefore(endDate)) {
                addPercentToAccount(timeToAddPercent);
                lastDateUpdate = newDateTime;
            }
        }
    ...
    private boolean isPeriodEnded(@NonNull LocalDateTime dateTime) {
